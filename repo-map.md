...
| ChatOps | `chat/` | Dependency-free Slack, Teams and Discord ingress, shared declarations, existing-principal federation, authorization, durable jobs, bounded replay and provider-native delivery. |
| Agents | `_agents/`, `agents.py` | Provider-neutral model backplanes, bounded runtime, routing, MCP tools, approvals, durable effects, memory, artifacts and observability. |
...
- Subsystem packages hold their own: `tests/orm/`, `tests/postgres/`, `tests/migrations/`, `tests/jobs/`, `tests/messaging/`, `tests/agents/`, `tests/chatops/`, `tests/storage/`, `tests/pagination/`, `tests/audit/`, `tests/health/`, `tests/flags/`, `tests/versioning/`, `tests/port/`, `tests/reactor/`, `tests/typegen/`.

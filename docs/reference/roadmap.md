...
| Surface | Status |
|---|---|
| Declaring infrastructure, and applying any of it | Not shipped. [`wreath.infra`](infra.md) reads an application and reports what it requires, and `wreath infra bundle` renders a gap-free plan plus an immutable OCI digest into an inspectable Compose runtime contract, original plan, and checksums. Both operations are offline. There is still no typed provider-resource declaration, Terraform/OpenTofu emission, live-state introspection or diff, image build, or `apply`; neither command imports a cloud SDK or opens a socket. An inference that is subtly wrong is worse than none, so the generated artifact stays provider-neutral and reviewable rather than pretending it can safely mutate an account. <!-- absent: wreath.infra.Stack --> |
...
| A native connection pool | Not shipped, and the stub that claimed otherwise is gone. `src/wreath/_native/postgres/pool.c` was an eight-line no-op — `wreath_pg_pool_init` took a module and returned zero — but it was listed in `setup.py`, compiled into the extension, listed in the sanitizer's source set, and *called from the module init chain*. It therefore passed MAP008, built clean, and ran on every import while doing nothing, so both the build and the init chain asserted a native pool was present. That assertion is what stopped anyone looking, and it is the shape `AGENTS.md` spends its length forbidding: success reported for work nobody did, surviving every gate. `wreath.postgres.Pool` is pure Python and stays that way for now. Its hot path is a `list.pop` and a `set.add` with no lock — the `asyncio.Condition` that used to be taken on every acquire *and* every release is gone, which was real per-request cost on the elided-call fast path guarding nothing, since neither sequence awaits and a single-threaded loop cannot interleave them. What a C pool would buy on top of that is unmeasured, and pricing it is the precondition for building it rather than the other way round: `uv run wreath-decomp` is where that number lives. <!-- absent: wreath._native.postgres.pool --> |
...
known optimistic-chrome limit, and for the same reason: the
...
SCIM ships without two optional parts of RFC 7644, and
`ServiceProviderConfig` reports each of them as unsupported rather than leaving
a client to discover it: **bulk operations** and **`ETag` resource versions** — the
...
did is worse than none. Bounded `sortBy`/`sortOrder` is implemented before
paging for both Users and Groups. `/Me` is likewise absent: it is an alias for a resource
...
## What is left of the native forward path for `wreath.edge`

**The forward path has shipped, and so has TLS.** `wreath.edge.serve()` is a
native protocol and terminates TLS in C — see
[the guide](../guides/edge.md#how-fast-honestly). Per physical core it forwards
33,300–33,900 plaintext requests a second against nginx's 30,900–31,700, and
37,100–38,200 with TLS against nginx's 36,800–37,600. What is on this page is
what it still refuses.

- **Retry onto a second upstream.** A failed attempt is a 502. Passive ejection
  still applies, so a failing origin stops being chosen — but the request that
  found it is not re-sent. `IDEMPOTENT` and the attempt loop live in
  `ReverseProxy`.
- **Protocol upgrades.** `Upgrade` is hop-by-hop and dropped, so a WebSocket
  handshake reaches the origin as an ordinary request. Neither path carries one.
- **HTTP/2 or HTTP/3 on either side**, and **SNI selection across hostnames**:
  a native TLS context is a single certificate.
- **Streamed request bodies.** Buffered under `max_body` and refused above it.
  Responses stream in both directions; the request half does not.
- **Kernel-side body relay.** Large bodies are copied through userspace where
  haproxy `splice()`s them, which is worth about 40% at 1 MB.

Two measurements worth keeping on this page, because both are traps rather than
trivia. Moving the header transform into C first — the obvious hot leaf —
measured 113.2 against a 110.1–117.2 baseline, which is nothing: the primitives
were already native and the cost was materialising the message at all. And
per-*process* CPU flattered this proxy at 22% cheaper than nginx when the honest
per-*core* figure is 7%, because roughly a third of a proxy's core is kernel

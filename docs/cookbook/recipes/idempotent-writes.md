...
(`POST`, `PUT`, `PATCH`, `DELETE`) are guarded; the key is scoped by method,
path, principal type, *and* principal id, with each component length-framed
before hashing, so two principals can't collide; an unauthenticated request is
skipped entirely rather than guarded, because there is no principal to scope it
by and a shared anonymous keyspace would let a client-chosen header replay one
caller's response to another; a concurrent duplicate

...
```bash
uv add 'wreath==0.5.0'          # portable framework and server
uv add 'wreath[linux]==0.5.0'   # Linux io_uring metal loop + native TLS transport
uv add 'wreath[h3]==0.5.0'      # Linux HTTP/3; http3 is an alias
```

`wreath[linux,h3]==0.5.0` selects both Linux capabilities. Confirm the resolved

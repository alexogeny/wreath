...
async def test_server_batches_window_update_after_consuming_body(make_driver):
    ...
    for offset in range(0, len(body), 4096):
        await d.feed_and_settle(
            support.encode_frame(support.DATA, 0, 1, body[offset : offset + 4096])
        )
    await d.feed_and_settle(support.encode_frame(support.DATA, support.FLAG_END_STREAM, 1, b""))
...
async def test_slow_application_withholds_receive_credit(make_driver):
    ...
    await d.feed_and_settle(
        support.build_headers_frame(
            1,
            support.request_headers(method=b"POST", extra=[(b"content-length", b"16384")]),
            end_stream=False,
        )
    )
    for _ in range(4):
...
async def test_data_exceeding_declared_content_length_is_protocol_error(make_driver):
    ...
    headers = support.build_headers_frame(
        1,
        support.request_headers(method=b"POST", extra=[(b"content-length", b"2")]),
        end_stream=False,
    )
    await d.feed_and_settle(headers)
    await d.feed_and_settle(
        support.encode_frame(support.DATA, support.FLAG_END_STREAM, 1, b"toolong")
    )
    frames = d.frames()
...
async def test_advertises_max_concurrent_streams(make_driver):
    ...
    d = make_driver(ok_app, config=ServerConfig(protocols=("h2",), max_concurrent_streams=42))
    d.connection_made()
    await d.settle()
    settings = [
        f for f in d.frames() if f.type == support.SETTINGS and not f.flags & support.FLAG_ACK
    ]
    assert settings
...
async def _blocked_driver(make_driver, app, *, initial_window=0):
    ...
    await d.feed_and_settle(support.build_headers_frame(1, support.request_headers(path=b"/")))
    return d
...
async def test_transport_pause_stops_data_framing_until_resume(make_driver):
    ...
    await d.feed_and_settle(
        support.build_headers_frame(1, support.request_headers(path=b"/paused"))
    )
...
async def test_connection_window_credit_is_shared_fairly(make_driver):
    ...
    await d.preface()
    d.protocol.pause_writing()
    await d.feed_and_settle(support.build_headers_frame(1, support.request_headers(path=b"/one")))
    await d.feed_and_settle(support.build_headers_frame(3, support.request_headers(path=b"/three")))
...
async def test_metal_rfc9218_urgency_preempts_lower_urgency(make_driver):
    ...
    await d.feed_and_settle(
        support.build_headers_frame(
            1, support.request_headers(path=b"/low", extra=[(b"priority", b"u=7, i")])
        )
    )
    await d.feed_and_settle(
        support.build_headers_frame(
            3, support.request_headers(path=b"/high", extra=[(b"priority", b"u=0, i")])
        )
    )
    d.protocol.resume_writing()
    await d.settle()
...
async def test_metal_rfc9218_priority_update_reorders_queued_stream(make_driver):
    ...
    await d.feed_and_settle(
        support.build_headers_frame(1, support.request_headers(path=b"/one", extra=priority))
    )
    await d.feed_and_settle(
        support.build_headers_frame(3, support.request_headers(path=b"/three", extra=priority))
    )
    update = support.encode_frame(0x10, 0, 0, (3).to_bytes(4, "big") + b"u=0, i")
...
async def test_metal_rfc9218_nonincremental_response_stays_sequential(make_driver):
    ...
    await d.settle()
    stream_order = [
        frame.stream_id for frame in d.frames() if frame.type == support.DATA and frame.payload
    ]
    assert stream_order
...
async def test_second_body_while_one_is_pending_is_rejected(make_driver):
    ...
    async def app(scope, receive, send):
        ...
        first = asyncio.ensure_future(
            send({"type": "http.response.body", "body": b"x" * 4096, "more_body": True})
        )

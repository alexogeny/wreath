...
def test_percent_decode_matches_urllib_unquote() -> None:
    ...
    for _ in range(500):
        ...
        for plus in (False, True):
            expected = urllib.parse.unquote_to_bytes(data.replace(b"+", b" ") if plus else data)
...
RFC6455_FRAMES = [
    ...
    pytest.param(bytes.fromhex("010348656c"), (False, 0x1, b"Hel", 5), id="fragment-1-of-2"),
    pytest.param(bytes.fromhex("80026c6f"), (True, 0x0, b"lo", 4), id="fragment-2-of-2"),
    pytest.param(bytes.fromhex("890548656c6c6f"), (True, 0x9, b"Hello", 7), id="unmasked-ping"),
    pytest.param(bytes.fromhex("8a0548656c6c6f"), (True, 0xA, b"Hello", 7), id="unmasked-pong"),
...
def test_the_email_anchor_agrees_with_the_written_out_parts() -> None:
    assert _email_multipart_parse(_multipart_body(b"BOUNDARY"), b"BOUNDARY") == (EXPECTED_PARTS)
...
PG_FLOAT_VALUES = [2**53 + 1, 2**63 - 1, 0, 1, -1, 1.5, -0.0, float("inf"), float("nan")]


@pytest.mark.parametrize("value", PG_FLOAT_VALUES, ids=repr)
@pytest.mark.parametrize("oid", [700, 701], ids=["float4", "float8"])
def test_float_binary_codec_matches_the_postgresql_wire_format(oid: int, value: object) -> None:
...
def test_every_declared_oid_has_a_written_out_answer_for_a_str() -> None:
    assert set(_STR_TEXT_ENCODING) | set(_STR_TEXT_REFUSAL) | {1560} == set(_PG_OIDS.values())


@pytest.mark.parametrize("value", STR_SAMPLES, ids=repr)
@pytest.mark.parametrize("oid", sorted(_STR_TEXT_ENCODING))
def test_a_str_parameter_is_encoded_with_its_columns_own_encoding(oid: int, value: str) -> None:
    native, pure = _pg_twins()
...
def test_a_str_parameter_is_refused_by_the_oids_that_cannot_hold_one(oid: int, value: str) -> None:
    native, pure = _pg_twins()
...
def test_loading_the_native_package_does_not_drag_in_asyncio() -> None:
    ...
    code = "import sys, wreath._native._core;print('asyncio' in sys.modules)"

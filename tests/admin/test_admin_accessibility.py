"""The generated pages pass wreath's own accessibility auditor.

Part of the suite rather than a manual step, because shipping a UI that fails
the gate this repository sells would be an embarrassment with a lint rule
attached. The rules are the same ones `wreath audit` applies to any other HTML;
nothing here is a special case for the admin.
"""
...
def _rows(model: type, count: int = 3) -> dict:
    return {
        n: model(
            id=n, name=f"Person {n}", email=f"p{n}@example.test",
            note="a note" if n % 2 else None, active=bool(n % 2),
        )
...
def _audit(name: str, response: Any) -> Report:
    report = Report()
    headers = {
        key.decode("latin-1"): value.decode("latin-1") for key, value in response.headers
    }
    audit_response(
        response.status, headers, response.body.decode("utf-8"),
        f"admin:{name}", report, scheme="https",
    )
    return report


async def test_the_auditor_can_fail(account_model: type) -> None:
    """Falsify the harness before trusting it: a page with no `lang`, no title
    and an unlabelled input must go red, or the assertions below prove nothing."""
    report = Report()
    audit_response(
...
async def test_every_admin_page_has_no_accessibility_errors(
...
async def test_every_admin_page_has_no_accessibility_warnings(
    ...
    """Warnings too. `--strict` fails on them, and an admin that only passes the
    loose gate is one someone will later be told to fix under deadline."""
    rendered = await _pages(account_model)
    report = _audit(page, rendered[page])
...
async def test_every_page_carries_the_scriptless_policy(account_model: type) -> None:
    """The deliberate CSP decision: no script at all, so no nonce is needed."""
...
async def test_a_row_value_is_escaped_into_the_page(account_model: type) -> None:
    """`wreath.templates` escapes by default; this asserts the admin did not
    route around it with `Markup` somewhere."""
    session = FakeSession({
        1: account_model(
            id=1, name="<script>alert(1)</script>", email="e@x.test",
            note=None, active=True,
        )
    })

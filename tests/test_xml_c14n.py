"""Exclusive XML Canonicalization 1.0, the form a signature is computed over.

The expectations here are derived from the W3C Exclusive XML Canonicalization
1.0 rules rather than from this implementation, so they fail if the
implementation drifts:

* empty elements are written as a start tag and an end tag, never self-closed
* namespace declarations come first, the default declaration before prefixed
  ones, prefixed ones sorted by prefix
* attributes follow, sorted by namespace URI then local name, with the
  no-namespace attributes (empty URI) first
* a namespace declaration is rendered only when the subtree **visibly
  utilizes** its prefix -- this is the whole difference between exclusive and
  inclusive canonicalization, and it is what lets a signed assertion survive
  being moved into a document that declares unrelated namespaces
...
def test_a_carriage_return_in_text_is_normalized_before_it_is_escaped() -> None:
    """Literal CR is line-ending normalization; ``&#xD;`` survives it.

    XML normalizes a literal CR in the source to LF at parse time, so only a
    character reference can put a real CR in the canonical output.
    """
...
def test_attribute_order_is_by_namespace_then_local_name() -> None:
    ...
    expected = (
        b'<a xmlns:m="urn:m" xmlns:z="urn:z" b="3" m:k="2" z:k="1"></a>'
    )
    assert parse(source).canonicalize() == expected
...
def test_an_unprefixed_attribute_does_not_utilize_the_default_namespace() -> None:
    """Unprefixed attributes are in no namespace, so they utilize nothing."""
...
def test_the_same_subtree_canonicalizes_identically_in_two_documents() -> None:
    """The property that makes a detached signature verifiable.

    An assertion signed inside one envelope must produce the same canonical
    bytes when it arrives inside a different envelope that happens to declare
    other namespaces. Inclusive canonicalization would fail this; exclusive is
    defined to pass it.
    """
    ...
    assert first.canonicalize(first.find_id("x")) == second.canonicalize(
        second.find_id("x")
    )
...
def test_an_inclusive_prefix_that_is_not_in_scope_is_simply_absent() -> None:
    ...
    child = doc.root.children[0]
    assert doc.canonicalize(child, inclusive_prefixes=("nope",)) == (
        b'<p:c xmlns:p="urn:p"></p:c>'
    )
...
def test_matches_stdlib_c14n_on_namespace_free_documents(source: bytes) -> None:
    """A cross-check against an implementation nobody here wrote.

    The two algorithms differ on namespace rendering, so this is restricted to
    documents with no namespaces at all -- where the escaping, attribute
    ordering and empty-element rules they share must produce the same bytes.
    """

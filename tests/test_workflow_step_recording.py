...
def test_a_failure_trigger_takes_both_failing_outcomes_and_not_completion() -> None:
    policy = WorkflowStepPolicy(triggers=(WorkflowStepTrigger(WorkflowStepTriggerKind.FAILURE),))
    assert policy.captures(workflow="c", step="s", outcome=WorkflowStepOutcome.RAISED, instance="i")
    assert policy.captures(
        workflow="c",
        step="s",
        outcome=WorkflowStepOutcome.COMPENSATION_FAILED,
        instance="i",
    )
    assert not policy.captures(
...
def test_a_named_workflow_trigger_ignores_another_workflows_steps() -> None:
    policy = WorkflowStepPolicy(
        triggers=(WorkflowStepTrigger(WorkflowStepTriggerKind.WORKFLOW, workflow="checkout"),)
    )
    assert policy.captures(
...
def test_two_instances_whose_keys_slug_alike_do_not_share_a_file(tmp_path) -> None:
    recorder = WorkflowStepRecorder(
        WorkflowStepPolicy(triggers=(WorkflowStepTrigger(WorkflowStepTriggerKind.FAILURE),)),
...
def test_an_overflowing_boundary_trace_refuses_the_recording(tmp_path) -> None:
    recorder = WorkflowStepRecorder(WorkflowStepPolicy(max_boundaries=2), directory=str(tmp_path))
...
async def test_the_failing_step_records_which_compensations_ran(tmp_path) -> None:
    ...
    with pytest.raises(RuntimeError):
        await workflow.run(store=InMemoryWorkflowStore(), key="checkout:4471", recorder=recorder)
...
async def test_an_unarmed_run_writes_nothing_and_still_completes(tmp_path) -> None:
    ...
    outcome = await workflow.run(store=InMemoryWorkflowStore(), key="checkout:1", recorder=recorder)
...
async def test_a_named_step_trigger_records_a_successful_step_too(tmp_path) -> None:
    recorder = WorkflowStepRecorder(
        WorkflowStepPolicy(
            triggers=(WorkflowStepTrigger(WorkflowStepTriggerKind.STEP, step="charge_card"),)
        ),
        directory=str(tmp_path),
    )
    workflow = _checkout(courier_fails=False)
...
async def test_the_record_carries_the_instances_trace_not_the_resuming_workers(
    ...
    await _checkout(courier_fails=False).resume(store=store, key="c:5", recorder=recorder)
    record = read_step_recording(sorted(tmp_path.glob("*.wfr1"))[0].read_bytes())
...
async def test_a_completed_step_records_no_error_at_all(tmp_path) -> None:
    recorder = WorkflowStepRecorder(
        WorkflowStepPolicy(
            triggers=(WorkflowStepTrigger(WorkflowStepTriggerKind.STEP, step="reserve_stock"),)
        ),
        directory=str(tmp_path),
    )
...
def test_a_plain_instance_key_keeps_its_own_name_in_the_file(tmp_path) -> None:
    assert _written(tmp_path, instance="order-4471") == ("checkout-order-4471-2-book_courier.wfr1")
...
def test_a_recorder_given_a_metadata_image_writes_that_one(tmp_path) -> None:
    ...
    image = MetadataImage(SCHEMA_VERSION, (), (), (NamedMeta(1, "session"),), *([()] * 8))
    recorder = WorkflowStepRecorder(
        WorkflowStepPolicy(triggers=(WorkflowStepTrigger(WorkflowStepTriggerKind.FAILURE),)),
        directory=str(tmp_path),
        image=image,

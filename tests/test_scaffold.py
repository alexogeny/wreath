...
def test_the_printed_next_steps_are_the_order_that_actually_works(
    ...
    steps = [
        line.strip()
        for line in capsys.readouterr().out.partition("next:")[2].splitlines()
        if line.strip()
    ]
...
def test_the_frontend_option_wires_typegen_to_the_route_table(
    ...
    assert (
        main(["typegen", f"{target.name}.app:app", "--output", str(output), "--react-query"]) == 0
    )
...
def test_the_database_option_compiles_its_model_once_a_dsn_is_supplied(
    ...
    assert _new(target, "--database", "postgres") == 0
    monkeypatch.setenv("WITHDSN_DATABASE_URL", "postgresql://wreath@127.0.0.1:55432/wreath_test")
    module = _load(target, importable)
...
def test_the_generated_react_app_typechecks_against_its_generated_client(
    ...
    assert (
        main(
            [
                "typegen",
                f"{target.name}.app:app",
                "--output",
                str(target / "web" / "src" / "api"),
                "--react-query",
            ]
        )
        == 0
    ...
    compiled = subprocess.run(
        [
            _node,
            str(_CONSUMER / "node_modules" / "typescript" / "bin" / "tsc"),
            "--noEmit",
            "--strict",
            "--target",
            "ES2022",
...
def test_the_tenanted_project_passes_its_own_preflight(
    ...
    importable.append(target)
    assert (
        main(["doctor", "preflight", f"{target.name}.app:app", "--env", str(target / ".env")]) == 0
    )

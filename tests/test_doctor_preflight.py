...
def test_the_route_finding_says_declaration_rather_than_claiming_the_route_is_open() -> None:
    detail = next(
        finding.detail for finding in preflight(build()).findings if finding.source == "routes"
    )
...
def test_the_command_exits_one_when_something_blocks(
    ...
    code = main(
        [
            "doctor",
            "preflight",
            "tests.preflight_app:app",
            "--settings",
            "tests.preflight_app:Settings",
        ]
    )
    assert code == 1
    ...
    assert (
        main(
            [
                ...
                "tests.preflight_app:app",
                "--settings",
                "tests.no_such_settings_module:Settings",
            ]
        )
        == 2
    )
...
def test_the_manifest_and_vocabulary_cover_five_hundred_routes() -> None:
    ...
    app = Wreath(require_access_declarations=True)
    app.configure_auth(BearerTokenBackend({}), vocabulary=AuthorizationVocabulary(actions_type))
    ...
    for index in range(500):
        app.get(f"/resources/{index}", operation_id=f"readResource{index}")(endpoint_for(index))

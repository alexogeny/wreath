...
requires_h3 = pytest.mark.skipif(not _http3_available(), reason="HTTP/3 backend not built")
...
async def test_http2_only_startup_and_shutdown():
    cert, key = _cert()
    server = await serve(
        _app, ServerConfig(port=0, lifespan="off", protocols=("h2",)), tls=TLSConfig(cert, key)
    )
    try:
        assert server.sockets
        assert server.datagram_addresses == ()
...
async def test_http3_only_startup_and_shutdown():
    cert, key = _cert()
    server = await serve(
        _app, ServerConfig(port=0, lifespan="off", protocols=("h3",)), tls=TLSConfig(cert, key)
    )
    try:
        assert server.datagram_addresses
        assert server.sockets == ()
...
async def test_combined_prefers_h2_when_configured_first():
    ...
    server = await serve(
        _app,
        ServerConfig(port=0, lifespan="off", protocols=("h2", "http/1.1")),
        tls=TLSConfig(cert, key),
    )
    try:
    ...
    try:
        ...
        with pytest.raises(OSError):
            await serve(
                _app,
                ServerConfig(
                    host="127.0.0.1", port=port, lifespan="off", protocols=("http/1.1", "h3")
                ),
                tls=TLSConfig(cert, key),
            )

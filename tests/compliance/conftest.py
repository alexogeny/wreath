...
def drive_request(request: bytes, *, app: Any = None, config: ServerConfig | None = None) -> bytes:
    """Feed `request` to the HTTP/1 protocol; return the raw response bytes.

    The one thing in the compliance suite that touches the server, so a build
    without `_server` is refused here rather than by each caller -- a test added

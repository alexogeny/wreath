"""Response security/compliance headers, codified against the runtime auditor.

These are the rules `wreath audit runtime` applies to a live service, mapping
each to its governing clause. A ResponseView stands in for a fetched response.
"""
...
def _view(status=200, scheme="https", headers=None, cookies=()):
    return ResponseView(status=status, scheme=scheme, surface="compliance",
...
def test_fetch_cors_wildcard_with_credentials() -> None:
    assert "cors-credentials" in _fired(_view(headers={

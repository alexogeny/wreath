...
async def test_arming_by_task_name_records_a_success_of_that_task_only(tmp_path):
    runner = _runner(tmp_path, AttemptTrigger(AttemptTriggerKind.TASK, task="import_herd"))
...
async def test_a_recording_whose_boundary_trace_overflows_is_refused_not_truncated(
    ...
    runner = _runner(tmp_path, AttemptTrigger(AttemptTriggerKind.FAILURE), max_boundaries=4)
...
async def test_a_recorder_that_cannot_write_counts_it_and_the_attempt_still_runs(
    ...
    assert any("state='ready'" in sql for sql, _ in runner._db.connection.calls)
...
def test_every_outcome_is_a_value_the_policy_can_be_asked_about(outcome):
    ...
    assert policy.captures(task="send", outcome=outcome, attempt=1, max_attempts=5, job_id=1) is (
        outcome is not AttemptOutcome.COMPLETED
    )

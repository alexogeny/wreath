...
def test_parameter_and_dynamic_routes_keep_wide_capability_masks() -> None:
    ...
    table.add_dynamic("/items/{item}", "GET", "{tenant}.example.test", hosted, (required,))
    table.compile()
    ...
    classification, ticket = table.classify_request("GET", "/items/42", "acme.example.test")
...
def test_application_compiles_more_than_64_distinct_capabilities() -> None:
    ...
    last = names[-1]
    mask = app._identity_mask(Identity(id="u1", roles=frozenset(), permissions=frozenset({last})))

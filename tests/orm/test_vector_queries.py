...
class Document(Model, table="documents", schema="app"):
    ...
    embedding: Mapped[list] = column(Vector(3), index="hnsw", index_ops="vector_cosine_ops")


class Documents(Queries[Document]):
    nearest = query().order_by(Document.embedding.cosine_distance(Param("q"))).limit(5)
...
def test_ordering_by_a_distance_and_a_column_together(registry: Registry) -> None:
    sql = _sql(
        registry,
        Document.select().order_by(Document.embedding.cosine_distance(QUERY), Document.id.desc()),
    )
...
async def live() -> Any:
    ...
    database = Database("vector-live", _DSN, pools={"write": PoolConfig(min_size=1, max_size=3)})
...
async def test_a_vector_column_hydrates_as_a_list_of_floats(live: Any) -> None:
    ...
    try:
        for _ in range(2):  # cold (text) then warm (binary)
            rows = await session.fetch(LiveDocument.select().where(LiveDocument.id == 2))

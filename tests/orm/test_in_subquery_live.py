...
_DDL = (
    ...
    f'CREATE TABLE "{_SCHEMA}"."species" (id bigint PRIMARY KEY, protection text NOT NULL)',
...
async def test_in_subquery_returns_the_right_rows_twice(registry: Any) -> None:
    query = Sighting.select().where(
        Sighting.species_id.in_(Species.select(Species.id).where(Species.protection == "open"))
    )
...
async def test_not_in_subquery_returns_the_complement_twice(registry: Any) -> None:
    query = Sighting.select().where(
        Sighting.species_id.not_in(Species.select(Species.id).where(Species.protection == "open"))
    )

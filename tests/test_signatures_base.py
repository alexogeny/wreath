...
def test_native_base_serializes_every_supported_parameter_shape():
    ...
    expected = b'"@signature-params": ("@method")' + _serialize_params(params).encode()
...
def test_empty_query_still_covers_a_question_mark():
    message = RequestMessage(method="GET", scheme="https", authority="h", path="/", query=b"")
...
def test_empty_query_adds_no_delimiter_to_request_targets():
    message = RequestMessage(method="GET", scheme="https", authority="example.com", path="/x")
    request_target = signature_base(message, (("@request-target", {}),), {"created": 1})
...
def test_a_well_formed_signature_input_parses():
    components, params = parse_input('sig1=("@method" "@path");created=1;keyid="k";alg="ed25519"')[
        "sig1"
    ]
...
def test_query_param_without_a_name_is_refused():
    message = RequestMessage(method="GET", scheme="https", authority="h", path="/", query=b"a=1")
...
def test_the_default_port_is_dropped_from_the_authority():
    def authority(value: str, scheme: str) -> str:
        message = RequestMessage(method="GET", scheme=scheme, authority=value, path="/")

...
class TestGrouped:
    ...
    async def test_one_row_per_group_keyed_by_the_grouping_value(self, session, database):
    ...
    async def test_it_orders_by_the_first_measure_then_the_key(self, session, database):
        await run(self._view(), session, database, [])
        assert "ORDER BY 2 DESC NULLS LAST, 1 ASC" in sql_of(database)

    async def test_it_fetches_one_more_than_the_ceiling_to_notice_overflow(self, session, database):
    ...
    async def test_it_refuses_past_the_ceiling_rather_than_truncating(self, session, database):
...
class TestPredicatesAndJoins:
    ...
    async def test_grouping_through_a_relation_joins_and_is_a_source(self, session, database):
...
class TestScatter:
    async def test_two_measures_per_entity_is_the_scatter_shape(self, session, database):
        view = (
            Aggregate(Trek)
    ...
    async def test_a_scatter_past_its_ceiling_refuses(self, session, database):
        view = Aggregate(Trek).measure(x=avg(Trek.distance_km)).by(Trek.paddock_id, limit=2)

...
class TestOneStatement:
    ...
    async def test_the_spine_gains_an_arm_rather_than_a_second_spine(self, session, database):
        view = series(bucket=Month).measure(n=count()).compare(previous=Year)
        await run(view, session, database, [])
        sql = sql_of(database)
...
class TestTheShiftHappensOnTheWallClock:
    ...
    async def test_the_filter_is_shifted_by_the_same_rule_as_the_spine(self, session, database):
        view = series(bucket=Day).measure(n=count()).compare(previous=Month)
        await run(view, session, database, [])
        sql = sql_of(database)
...
class TestOverlapIsRefused:
    ...
    async def test_a_twenty_five_hour_day_does_not_trip_the_guard(self, session, database):
    ...
    async def test_a_month_end_shift_clamps_rather_than_overflowing(self, session, database):
        view = series(bucket=Day).measure(n=count()).compare(previous=Month)
        await run(
            view,
            session,
...
class TestSurvivors:
    async def test_they_are_ranked_over_the_primary_period_only(self, session, database):
        view = (
            series(bucket=Day).measure(n=count()).by(Trek.paddock_id, top=3).compare(previous=Month)
        )
        await run(view, session, database, [], range=Range(utc(2026, 1, 1), utc(2026, 1, 8)))
        sql = sql_of(database)
...
class TestTheEnvelope:
    ...
    async def test_a_period_that_matched_nothing_is_present_and_empty(self, session, database):
        view = series().measure(n=count()).compare(previous=Month)
    ...
    async def test_fill_applies_to_the_comparison_too(self, session, database):
        view = series().measure(n=count(), pace=avg(Trek.distance_km)).compare(previous=Month)
        rows = [
    ...
    async def test_grouped_series_split_by_period(self, session, database):
        view = series().measure(n=count()).by(Trek.paddock_id, top=2).compare(previous=Month)
        rows = [
    ...
    async def test_a_view_that_does_not_compare_has_no_comparison(self, session, database):

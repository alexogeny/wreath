...
def test_generator_thread_count_is_recorded_for_every_run() -> None:
    ...
    assert _generator_threads(args.generator_threads, args.connections or args.concurrency) == 3
...
def test_background_scenarios_are_skipped_when_there_is_more_than_one_worker() -> None:
    ...
    unsupported = next(n for n, s in SCENARIOS.items() if not s.supports("axum"))

"""First-class request correlation policy.
...
```python
app.configure_http_policy(HttpPolicy(request_id=RequestIdPolicy()))
...
def request_id(request: Request) -> str:
    """Return the id `RequestIdPolicy` assigned to this request.
    ...
    Raises:
        RuntimeError: No id was assigned, so `RequestIdPolicy` is not mounted.
    """
    context = request._context
    native = None if context is None else context.policy_request_id
    if native is not None:
        return native.decode("ascii")
    value = request.state.get(_STATE_KEY)
    if value is None:
        raise RuntimeError("RequestIdPolicy has not assigned an id to this request")
    return value
...
class RequestIdPolicy:
    ...
    Global policy, so every response is correlated, including route misses
    ...
    def describe(self) -> Any:
        ...
        A policy built with `echo=False` emits nothing, and saying so is
    ...
    def _ingress_sync(self, request: Request) -> None:
        ...
        if value is None:
            ...
            # performs a syscall per call, and this stage runs on every request.
    ...
    def _egress_inplace(self, request: Request, response: Any) -> None:
        ...
        No id is assigned when a `_ingress` stage ahead of this one short-circuited
...
__all__ = ["RequestIdPolicy", "request_id"]

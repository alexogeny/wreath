"""First-class signed double-submit CSRF policy.
...
exposed to this and does not need the policy:

```python
app.configure_http_policy(HttpPolicy(
    csrf=CsrfPolicy(secret=SECRET, trusted_hosts=["app.example"]),
))
...
def csrf_token(request: Request) -> str:
    """Return the CSRF token `CsrfPolicy` prepared for this request.
    ...
    `_egress` still writes the cookie, because minting sets the same flag the
    ...
    Raises:
        RuntimeError: No token was prepared for this request, and no policy
            was recorded that could mint one -- the policy is not installed,
            or the `exempt` predicate excused this request.
    """

    context = request._context
    native = None if context is None else context.policy_csrf_token
    if native is not None:
        return native
    token = request.state.get(_STATE_TOKEN)
    if token is not None:
        return token
    policy = request.state.get(_STATE_MINTER)
    if policy is None:
        raise RuntimeError("CsrfPolicy has not prepared a token for this request")
    return policy._mint_for(request)
...
class CsrfPolicy:
    ...
    **The token below is the fallback**, for a client that sent no
    ...
    browser through the cookie, which `_egress` writes whenever the token was
    ...
    calls `csrf_token`, which mints one then and leaves `_egress` to write the
    ...
    this policy self-contained; leaving it empty means
    `TrustedHostPolicy` must be mounted instead. Behind a TLS-terminating
    proxy, `ProxyPolicy` must run first or the expected origin is
    ...
    Args:
        ...
        trusted_hosts: Host values accepted. Empty defers to `TrustedHostPolicy`.
    ...
    def __init__(
        ...
        *,
        ...
        # something was `TrustedHostPolicy` -- a dependency between two
        # policies that nothing stated and nothing enforced. Naming the hosts
        # here makes this policy self-contained; leaving it empty keeps the
        # previous behaviour, and the guide says to mount TrustedHostPolicy.
    ...
    def describe(self) -> Any:
        """The token header this policy requires, and the 403 without it.
    ...
    def _mint_for(self, request: Request) -> str:
        ...
        Only reached through `csrf_token`, on the path where `_ingress` skipped
        minting. Sets the same state `_egress` reads, so the cookie is written
    ...
    def _egress_inplace(self, request: Request, response) -> None:
        ...
        `Vary` another policy already set is not overwritten.
        """

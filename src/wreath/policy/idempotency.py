...
policy remembers the first response and replays it for any repeat of the
same key.
...
Even then, remember what this policy is: **response replay**. It saves the
...
def _replayable_headers(
    ...
    return tuple(
        ...
        # the worst possible moment for this policy to raise.
...
class PostgresIdempotencyStore:
    ...
    exclusion. Everything else here is this policy's payload, a status,
    ...
    def schema_database(self) -> Any:
        ...
        and hands it to `IdempotencyPolicy` -- so it cannot know which
...
class IdempotencyPolicy:
    ...
    **An unauthenticated request is not guarded at all.** The handler runs
    exactly as it would without this policy, and a retry re-runs it. That
    ...
    500 by the time this policy sees it, so it releases the key too — a
    ...
    this policy did not act on; silence there is how "idempotency works in
    ...
    def __init__(
        ...
        *,
        ...
        #: Requests that sent a key this policy did not act on -- an
    ...
    def describe(self) -> Any:
        """The key this policy reads, and the 409 a concurrent repeat gets.
    ...
    def _key(self, request: Request) -> str | None:
        ...
        **Idempotency requires an authenticated principal.** The key is scoped
        ...
        the handler runs exactly as it would without this policy, and the
    ...
    async def action(self, request: Request):
        ...
        `principal_key` at construction for exactly this reason; this policy

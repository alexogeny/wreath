"""First-class trusted forwarding policy for deployments behind a proxy.
...
`SecurityHeadersPolicy` gates HSTS on an HTTPS scheme and emits nothing, and
`CsrfPolicy` builds its expected origin as `http://host` while the browser
sends `https://host` and rejects every unsafe request.

This policy restores the truth, but only from proxies that were explicitly
configured:
...
class ProxyPolicy:
    ...
    `Host` *header* rather than a scope entry, because `TrustedHostPolicy`
    and `CsrfPolicy` read the header.

    Mount it before anything that reads scheme, host, or client -- a negative
    priority puts it first. Without it, `SecurityHeadersPolicy` sees an
    `http` scheme and emits no HSTS, and `CsrfPolicy` builds an expected
    ...
    def _ingress_sync(self, request: Request) -> None:
        ...
        Always returns None; this stage never short-circuits. A request whose

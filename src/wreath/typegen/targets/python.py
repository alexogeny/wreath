...
from ..model import ApiModel, Model, Operation, Parameter, TypeRef
...
def _needs_literal(api: ApiModel) -> bool:
    def walk(ref: TypeRef) -> bool:
        return ref.kind == "literal" or any(walk(a) for a in ref.arguments)
...
def _models_module(api: ApiModel) -> str:
    ...
    literal = ", Literal" if _needs_literal(api) else ""
    return f'''"""Generated by wreath typegen. Do not edit.

Response and request shapes for {api.title} {api.version}.
"""
...
def _method_source(operation: Operation, api: ApiModel) -> str:
    ...
    if header:
        ...
        for parameter in header:
            lines.append(f"        if {parameter.python_name} is not None:")
            lines.append(
                f'            headers.append((b"{parameter.wire_name.lower()}", '
...
A typed client for {api.title} {api.version}.

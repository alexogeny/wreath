...
of scope. Colour contrast (1.4.3) is Tier 2, not here.
...
def _f(rule_id, severity, surface, message, reference, node=None, suggestion=""):
    return Finding(
        rule_id, severity, surface, message, reference, node.loc if node else "", suggestion
    )
...
def _accessible_name(node: Node) -> bool:
    return (
        bool(node.text)
        or node.has_attr("aria-label")
        or node.has_attr("aria-labelledby")
        or bool(node.tag == "img" and node.attr("alt"))
        or bool(node.attr("title"))
    )
...
def html_lang(root, surface) -> Iterator[Finding]:
    ...
    if not (html.attr("lang") or "").strip():
        yield _f(
            ...
            "WCAG 3.1.1",
            html,
            suggestion='add lang="en" (or the document language)',
        )
    ...
    for control in root.find_all("input"):
        ...
        if not (control.attr("alt") or "").strip():
            yield _f(
                ...
                Severity.ERROR,
                surface,
                '<input type="image"> has no text alternative',
                "WCAG 1.1.1",
                control,
                suggestion='add a non-empty alt="…" (or aria-label)',
            )
    ...
    for area in root.find_all("area"):
        ...
        if not (area.attr("alt") or "").strip():
            yield _f(
                ...
                Severity.ERROR,
                surface,
                "<area href> image-map hotspot has no alt text",
                "WCAG 1.1.1",
                area,
                suggestion='add a non-empty alt="…" describing the target',
            )
    ...
    for frame in root.find_all("iframe"):
        ...
        if not has_name:
            yield _f(
                ...
                Severity.ERROR,
                surface,
                "<iframe> has no title (accessible name)",
                "WCAG 4.1.2",
                frame,
...
def control_label(root, surface) -> Iterator[Finding]:
    ...
    for node in root.walk():
        ...
        has_name = (
            node.has_attr("aria-label")
            or node.has_attr("aria-labelledby")
            or node.has_attr("title")
            or wrapped
...
def landmarks(root, surface) -> Iterator[Finding]:
    if root.first("html") is None:
        return
    has_main = root.first("main") is not None or any(n.attr("role") == "main" for n in root.walk())
    if not has_main:
        yield _f(
            "landmarks",
            Severity.WARN,
        ...
        for name in node.attrs:
            if name.startswith("aria-") and name not in _ARIA_ATTRS:
                yield _f(
                    ...
                    Severity.WARN,
                    surface,
                    f"unknown ARIA attribute {name!r}",
                    "WCAG 4.1.2",
                    node,
    ...
    for node in root.walk():
        ...
        if value > 0:
            yield _f(
                ...
                "WCAG 2.4.3",
                node,
                suggestion="use tabindex=0 or restructure the DOM",
            )
    ...
    for node in root.find_all("audio", "video"):
        if node.has_attr("autoplay") and not node.has_attr("muted"):
            yield _f(
                ...
                Severity.WARN,
                surface,
                f"<{node.tag} autoplay> plays sound with no control",
                "WCAG 1.4.2",
                node,
                suggestion="add muted, or provide a pause/stop control",
            )

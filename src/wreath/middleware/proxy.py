...
:class:`~wreath.middleware.SecurityHeadersMiddleware` gates HSTS on an HTTPS scheme
and emits nothing, and :class:`~wreath.middleware.CSRFMiddleware` builds its
expected origin as ``http://host`` while the browser sends ``https://host`` and
rejects every unsafe request.

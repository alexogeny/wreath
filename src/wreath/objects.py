...
Phase 1 (this module): the :class:`ObjectStore` protocol, a pathlib-like
:class:`ObjectPath`, an in-process :class:`MemoryObjectStore` (the dev/test twin the
apps hand-roll), a symlink-safe :class:`LocalObjectStore` backend (built on the same
``openat`` containment as static files), and incremental Zip64 :func:`zip_stream`.

Phase 2 (also here): :class:`S3ObjectStore` — the S3 REST API over ``http_client`` signed
...
class ObjectPath:
    """Immutable pathlib-like handle bound to a :class:`ObjectStore` and a key.
...
class _PathIO:
    """Convenience async context manager over a :class:`ObjectPath`.
    ...
    writes use :meth:`ObjectStore.write_stream` directly.
    """
...
async def file_chunks(data: bytes, *, chunk_size: int = 1 << 20) -> AsyncIterator[bytes]:
    """Bridge an already-read upload body into :meth:`ObjectStore.write_stream`.

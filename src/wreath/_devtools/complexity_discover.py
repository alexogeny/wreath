...
def infer_kinds(fn: ast.FunctionDef | ast.AsyncFunctionDef) -> dict[str, str | None]:
    ...
    for node in _owned_statements(fn):
        if isinstance(node, ast.AnnAssign) and isinstance(node.target, ast.Name):
            kinds[node.target.id] = _annotation_kind(node.annotation) or (
                kind_of(node.value) if node.value is not None else None
            )
            ...
            for target in node.targets:
                if isinstance(target, ast.Name):
                    ...
                    kinds[target.id] = (
                        None if (target.id in kinds and kinds[target.id] != kind) else kind
                    )
    return kinds
...
class _PythonScanner(ast.NodeVisitor):
    ...
    def _report(self, node: ast.AST, code: str, message: str, confidence: str = "high") -> None:
        line = getattr(node, "lineno", 0)
        waiver = re.compile(rf"#\s*complexity:\s*allow\s+{re.escape(code)}\s+--\s*\S")
        if any(
    ...
    def visit_FunctionDef(self, node) -> None:
        ...
        if not any("cache" in item for item in decorators) and _has_branching_recursion(node):
            self.recursion_findings.append(
                Finding(
                    file=self.display,
                    line=node.lineno,
                    code="SL-RECURSE",
    ...
    def visit_For(self, node) -> None:
        ...
        if iterated and any(loop.iterated == iterated and not loop.constant for loop in self.loops):
            self._report(
                node,
                "SL-NEST-SAME",
                f"nested loop re-iterates `{iterated}`, already being iterated outside",
                confidence="high",
            )
    ...
    def visit_Compare(self, node) -> None:
        for op, comparator in zip(node.ops, node.comparators, strict=True):
            ...
            if self._depth() >= 1 and kind in SEQUENCED and name and not self._loop_local(name):
                self._report(
                    node, "SL-IN-LOOP", f"membership test against `{name}` (a {kind}) inside a loop"
                )
        self.generic_visit(node)
    ...
    def _check_call(self, node: ast.Call) -> None:
        ...
        if isinstance(func, ast.Attribute):
            ...
            if receiver and not self._loop_local(receiver):
                if front:
                    ...
                    self._report(
                        node,
                        "SL-LINEAR-METHOD",
                        f"`{receiver}.{method}()` inside a loop: {RECEIVER_LINEAR_METHODS[method]}",
                    )
            if method in ARGUMENT_LINEAR_METHODS and node.args:
                ...
                receiver_supports_method = (
                    ...
                    or method == "extend"
                    and receiver_kind == "list"
                    or method == "update"
                    and receiver_kind in HASHED
                )
                if (
                    receiver_supports_method
                    and argument
                    ...
                    self._report(
                        node,
                        "SL-LINEAR-METHOD",
                        f"`{method}({argument})` inside a loop: {ARGUMENT_LINEAR_METHODS[method]}",
                    )
        ...
        elif isinstance(func, ast.Name) and func.id in LINEAR_FUNCS and len(node.args) == 1:
            argument = _name_of(node.args[0])
            if (
                argument
                ...
                self._report(
                    node,
                    "SL-LINEAR-CALL",
                    f"`{func.id}({argument})` inside a loop: {LINEAR_FUNCS[func.id]}",
                )
    ...
    def visit_Subscript(self, node) -> None:
        if self._depth() >= 1 and isinstance(node.slice, ast.Slice):
            ...
            bounded_prefix = (
                lower is None and isinstance(upper, ast.Constant) and isinstance(upper.value, int)
            )
            bound = upper if lower is None else lower if upper is None else None
            invariant_bound = bound is None or not any(
                self._loop_local(item.id) for item in ast.walk(bound) if isinstance(item, ast.Name)
            )
            if (
                name
                ...
                self._report(
                    node, "SL-SLICE-LOOP", f"slice of `{name}` inside a loop copies each time"
                )
        self.generic_visit(node)
    ...
    def _comprehension(self, node) -> None:
        ...
        for generator in node.generators:
            ...
            if (
                ...
                self._report(node, "SL-COMP-LOOP", f"comprehension over `{name}` inside a loop")
            if name and name in seen and self.kinds.get(name) in HASHED | SEQUENCED:
                self._report(node, "SL-COMP-NEST", f"comprehension re-iterates `{name}`")
            if name:
                seen.add(name)
...
_FUNC = re.compile(r"^(?:[A-Za-z_][A-Za-z0-9_ *]*\b\s+)?(\w+)\s*\([^;]*\)\s*\{?\s*$")
...
def scan_c(path: Path, root: Path) -> list[Finding]:
    ...
    def found(
        code: str, message: str, at_depth: int, *, number: int, func: str, raw: str
    ) -> Finding:
    ...
    for number, raw in enumerate(
        ...
        if is_loop:
            ...
            fixed_bound = bool(bound and re.fullmatch(r"[A-Z_][A-Z0-9_]*", bound))
...
def _source_paths(root: Path) -> list[Path]:
    ...
    return sorted(path for path in root.rglob("*") if path.suffix in {".py", ".c", ".h"})
...
def _source_fingerprints(root: Path) -> dict[str, str]:
    ...
    return {str(path.relative_to(root)): _source_fingerprint(path) for path in _source_paths(root)}
    ...
    *,
    ...
    for path in _source_paths(root):
        ...
        if cached is not None and cached[0] == _source_fingerprint(path):
            ...
            continue
        per_file.append(scan_python(path, root) if path.suffix == ".py" else scan_c(path, root))

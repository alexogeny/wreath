...
def _render_items(
    ...
    for item in items:
        if isinstance(item, Page):
            ...
            parts.append(
                f'<a class="{css}" href="{_esc(_relative(current, target))}"{aria}>'
                f"{_esc(item.title)}</a>"
            )
        else:
...
def _nav_context(
    ...
    for owner, (item, landing) in enumerate(image.entries):
        ...
        entries.append(
            f'<a href="{_esc(href)}"{' class="active"' if active else ""}'
...
def build(site: Site, root: Path | None = None) -> BuildReport:
    ...
    for page, orphan in queue:
        ...
        if not src.is_file():
            ...
            errors.append(f"{'orphan' if orphan else 'nav'} page missing on disk: {page.source}")
            continue
        ...
        command_findings = command_checker.check_page(text, page.source)
        ...
        text, card_tokens = cards.extract(text)
        ...
        text, plate_tokens = plate.extract(text, source_dir, errors if site.strict else warnings)
        ...
        if apidoc.has_directives(text):
            ...
            text = apidoc.expand(text, page.source, errors if site.strict else warnings)
        ...
        (rendered_orphans if orphan else rendered_pages).append(
            _RenderedPage(
                page,
                _output_path(page.source),
                rendered.title
                or hero.title_of(hero_tokens)
                or plate.title_of(plate_tokens)
                or page.title,
                html,
                rendered.toc,
                frozenset(e.slug for e in rendered.toc),
                description,
                keywords,
                boost,
                aliases,
    ...
    slugs_by_page = {rp.out_rel: rp.slugs for rp in [*rendered_pages, *rendered_orphans]}
    ...
    for rp in rendered_orphans:
        _check_page(rp.html, rp.out_rel, reachable, slugs_by_page, sink, label="orphan")
    ...
    repo_html = theme.repo_link(repo.describe(site.repo, warnings) if site.repo else None)
    ...
    (output_dir / "assets" / "search-index.json").write_text(
        json.dumps({"p": index_pages, "s": index_sections}, separators=(",", ":")), encoding="utf-8"
    )
...
def _write_404(
    ...
    context = PageContext(
        ...
        section_href=section_href,
        toc_html="",
        css_href=_relative("404.html", _CSS_PATH),
...
def _footer(
    current: str,
    prev: _RenderedPage | None,
    nxt: _RenderedPage | None,
    site: Site,
) -> str:
    """Prev/next through nav order, and the page's own source when it has one."""
    left = (
        f'<a class="nav-prev" href="{_esc(_relative(current, prev.out_rel))}">'
        f'<span class="dir">&larr; Previous</span>'
        f'<span class="title">{_esc(prev.title)}</span></a>'
        if prev
        else "<span></span>"
    )
    right = (
        f'<a class="nav-next" href="{_esc(_relative(current, nxt.out_rel))}">'
        f'<span class="dir">Next &rarr;</span>'
        f'<span class="title">{_esc(nxt.title)}</span></a>'
        if nxt
        else "<span></span>"
    )
    meta = ""
...
def _check_page(
    ...
    current: str,
    known: set[str],
    slugs_by_page: dict[str, frozenset[str]],
    sink: list[str],
    label: str = "",
) -> None:
...
def _excluded(rel: str, patterns: tuple[str, ...]) -> bool:
    ...
    return any(
        fnmatch(rel, pattern) or fnmatch(rel, pattern.rstrip("/") + "/*") for pattern in patterns
    )

...
def parse_target(spec: str, *, label: str, default_attribute: str | None = None) -> Target:
    """Parse one import target, refusing malformed names before importing."""
    if spec.count(":") > 1:
        raise ValueError(f"{label} target {spec!r} must be spelled module:attribute")
...
def load_target(
    ...
    *,
    ...
    target = parse_target(spec, label=label, default_attribute=default_attribute)
    try:
        module = importlib.import_module(target.module)
    except Exception as error:
        if not catch_all_import_errors and not isinstance(error, ImportError):
            raise
        raise ValueError(f"could not import {label} module {target.module!r}: {error}") from error
    try:
        return getattr(module, target.attribute)
    except AttributeError as error:
        raise ValueError(
            f"{label} module {target.module!r} has no attribute {target.attribute!r}"
        ) from error

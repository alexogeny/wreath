 ...
 *
 * Harvested unchanged from arm 3. Own translation unit, -mavx2 -mvpclmulqdq,
 * reached only through the runtime probe. VPCLMULQDQ applies the carry-less
 * multiply independently to each 128-bit lane, so a ymm is two folds with one
 * instruction: the same constants work unchanged, and the two lanes are
 * combined at the end by folding lane 0 forward the 16 bytes that separate
 * them.
 */
#include <immintrin.h>
...
uint32_t wreath_gzip_decoder_crc32_vpclmul(uint32_t crc, const void *buf, size_t len) {
  ...
  if (len >= 128) {
    ...
    c = wreath_gzip_decoder_crc32_raw_slice16(0, tmp, 16);
    _mm256_zeroupper();
  }
  c = wreath_gzip_decoder_crc32_raw_slice16(c, p, len);
  return ~c;
}
